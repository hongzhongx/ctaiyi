...
describe('client', function () {
  ...
  it('should make rpc call', async () => {
    const result = await client.call('baiyujing_api', 'get_accounts', [['initminer']]) as any[]
  ...
  it('should handle rpc errors', async () => {
    try {
      await client.call('baiyujing_api', 'i_like_turtles')
  ...
  it('should format rpc errors', async () => {
    const tx = { operations: [['siming_update', {}]] }
    try {
      await client.call('baiyujing_api', 'broadcast_transaction', [tx])
      assert(false, 'should not be reached')
    }
    catch (error) {
      assert.equal(error.name, 'RPCError')
  ...
  it('should retry and timeout', async function () {
    ...
    aclient.address = 'https://hongzhongx.github.io/ctaiyi/FAIL'
